...
class StringTokenizer implements Enumeration {
    ...
    public static int Character_charCount(int codePoint) {
    ...
    public static boolean Character_isLowSurrogate(char ch) {
    ...
    public static int Character_codePointAtImpl(char[] a, int index, int limit) {
        char c1 = a[index++];
        if (Character_isHighSurrogate(c1)) {
            if (index < limit) {
                char c2 = a[index];
                if (Character_isLowSurrogate(c2)) {
    ...
    public int String_codePointAt(String _str, int index) {
        if ((index < 0) || (index >= _str.length())) {
            throw new StringIndexOutOfBoundsException(index);
        }
        int offset = 0;
        return Character_codePointAtImpl(_str.toCharArray(), offset + index, offset + _str.length());
    }    
    ...
    private void setMaxDelimCodePoint() {
        ...
        int c;
        int count = 0;
        for (int i = 0; i < delimiters.length(); i += Character_charCount(c)) {
            c = delimiters.charAt(i);
            if (c >= Character.MIN_VALUE && c <= Character.MAX_VALUE) {
                c = String_codePointAt(delimiters,i);
                hasSurrogates = true;
            }
            if (m < c)
                m = c;
        ...
        if (hasSurrogates) {
            ...
            for (int i = 0, j = 0; i < count; i++, j += Character_charCount(c)) {
                c = String_codePointAt(delimiters,j);
    ...
    private int skipDelimiters(int startPos) {
        ...
        while (!retDelims && position < maxPosition) {
            if (!hasSurrogates) {
                ...
                int c = String_codePointAt(str, position);
                if ((c > maxDelimCodePoint) || !isDelimiter(c)) {
                    break;
                }
                position += Character_charCount(c);
            }
        }
        return position;
    }
